...
DllExport BOOL SetHooks(DWORD thread_id, UINT UpdateMsg, UINT CopyMsg, UINT MouseMsg,BOOL ddihook)
{
	...
	// If we already running, just switching thread id to new one
	if (vnc_thread_id)
	{
		vnc_thread_id = thread_id;
		return TRUE;
	}
...
#ifdef _MSC_VER 
		_RPT0(_CRT_WARN, "vncHooks : Hooking started");
#endif
		return TRUE;
	}
	else
...
DllExport BOOL UnSetHooks(DWORD thread_id)
{
	...
	//if (thread_id != vnc_thread_id) // Max: do not think this check is needed
	//	return FALSE;
...
#ifdef _MSC_VER 
		_RPT0(_CRT_WARN, "vncHooks : Hooking stopped");
#endif
