...
inline void doKeyboardEvent(BYTE vkCode, DWORD flags, vncServer *server = NULL) 
{
	if (NULL != server)
	{
		if ((server->IsInputProxyNeeded() || server->GetDesktopPointer()->m_lockFix->Active()) && NULL != server->GetSrvCommunicator())
		{
			SSendKeyEventMsg msg;
			msg.flags = flags;
			msg.vkCode = vkCode;
			msg.extraInfo = MESS_MARKER;
			msg.toWinlogon = server->GetDesktopPointer()->m_lockFix->Active();
			server->GetSrvCommunicator()->SendMsg(SRVCOMM_SEND_KBD_INPUT, reinterpret_cast<char*>(&msg), sizeof(msg));
			return;
		}
	}
	
	//Log.Add(_MESSAGE_,_T("------- Key vkCode(%d), scanCode(%d), flags(%d)"), vkCode, MapVirtualKey(vkCode, 0), flags);

	keybd_event(vkCode, MapVirtualKey(vkCode, 0), flags, MESS_MARKER);
}
...
public:
  ...
  for (int j = 0; j < sizeof(latin1DeadChars); j++) 
  {
    ...
    if (s != -1) {
      ...
      if (nchars < 0) 
	  {
		...
		Log.Add(_MESSAGE_, "Found dead key 0x%x '%c'", latin1DeadChars[j], latin1DeadChars[j]);
  ...
  void keyEvent(rdr::U32 keysym, bool down, vncServer *server)
  {
	...
	if (keysymDead!=0 && down)
	{
	 ...
	 if (s == -1)
	 {
        ...
        // not a single keypress - try synthesizing dead chars.
			{
			  ...
			  keybd_event(VK_MENU,MapVirtualKey( VK_MENU, 0 ), 0 ,MESS_MARKER);
			  keybd_event(VK_NUMPAD0+a0, MapVirtualKey(VK_NUMPAD0+a0, 0), 0, MESS_MARKER);
			  keybd_event(VK_NUMPAD0+a0, MapVirtualKey(VK_NUMPAD0+a0, 0),KEYEVENTF_KEYUP,MESS_MARKER);
			  keybd_event(VK_NUMPAD0+a1, MapVirtualKey(VK_NUMPAD0+a1, 0),0,MESS_MARKER);
			  keybd_event(VK_NUMPAD0+a1, MapVirtualKey(VK_NUMPAD0+a1, 0),KEYEVENTF_KEYUP, MESS_MARKER);
			  keybd_event(VK_NUMPAD0+a2, MapVirtualKey(VK_NUMPAD0+a2, 0) ,0, MESS_MARKER);
			  keybd_event(VK_NUMPAD0+a2, MapVirtualKey(VK_NUMPAD0+a2, 0),KEYEVENTF_KEYUP, MESS_MARKER);
			  keybd_event(VK_MENU, MapVirtualKey( VK_MENU, 0 ),KEYEVENTF_KEYUP, MESS_MARKER);
			  return;
			  }
        }
      ...
      doKeyboardEvent(vkCode, down ? 0 : KEYEVENTF_KEYUP, server);
      ...
      if (vkMap.find(keysym) == vkMap.end()) 
	  {
		...
		Log.Add(_ERROR_, "ignoring unknown keysym %d",keysym);
        return;
      }
...
//      vnclog.Print(LL_INTINFO,
  //                "keyboard key: keysym %d(0x%x) vkCode 0x%x ext %d down %d\n",
    //               keysym, keysym, vkCode, extendedMap[keysym], down);
      ...
      doKeyboardEvent(vkCode, flags, server);
    }
  }
