...
UINT
vncEncodeZlibHex::EncodeRect(BYTE *source, boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/, BYTE *dest, const RECT &rect)
{
	...
	try
	{
		outstream->Send2Queue( (char *)dest, sz_rfbFramebufferUpdateRectHeader);
	}
...
//	AddToQueu((BYTE *)dest,sz_rfbFramebufferUpdateRectHeader,outConn);
	...
	{
	case 8:
		retval= EncodeHextiles8(source, dest, outstream, rect.left, rect.top, rectW, rectH);
		SendZlibHexrects(outstream);
		return retval;
    case 16:
		retval= EncodeHextiles16(source, dest, outstream, rect.left, rect.top, rectW, rectH);
		SendZlibHexrects(outstream);
		return retval;
    case 32:
		retval= EncodeHextiles32(source, dest, outstream, rect.left, rect.top, rectW, rectH);
		SendZlibHexrects(outstream);
		return retval;
    }
...
//	unsigned int out_len;
	...
	if ( compressor->total_in == ZLIBHEX_COMP_UNINITED )
	{
		...
		if ( deflateResult != Z_OK )
		{
			...
			Log.Add(_ERROR_, "deflateInit2 returned error:%d:%s", deflateResult, compressor->msg);
			return -1;
		}
	...
	if ( deflateResult != Z_OK )
	{
		...
		Log.Add(_ERROR_, "deflate returned error:%d:%s", deflateResult, compressor->msg);
		return -1;
	}
...
vncEncodeZlibHex::EncodeHextiles##bpp(BYTE *source, BYTE *dest,				\
				  boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/, int rx, int ry, int rw, int rh)			\
{																			\
    int x, y, w, h;															\
    int rectoffset, destoffset;												\
    int encodedBytes, compressedSize;										\
	CARD16* card16ptr;														\
	CARD##bpp bg, fg = 0 /*vc++ 7.0 bugfix*/, newBg, newFg;											\
	BOOL mono, solid;														\
	BOOL validBg = FALSE;													\
	BOOL validFg = FALSE;													\
	int subEncodedLen;														\
	...
	{																		\
		...
		{																	\
			...
			AddToQueu((BYTE *)dest,destoffset,outstream);				        \
...
void
vncEncodeZlibHex::AddToQueu(BYTE *source,int sizerect, boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
	if (m_Queuelen+sizerect>(MaxQueuebufflen)) SendZlibHexrects(outstream);
	if (sizerect>(MaxQueuebufflen)) 
		{
			try
			{
				outstream->Send2Queue( (char *)source,sizerect);
			}
			catch(CStreamException&)
			{
			}
...
void
vncEncodeZlibHex::SendZlibHexrects(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
...
void
vncEncodeZlibHex::LastRect(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
	SendZlibHexrects(outstream);
}