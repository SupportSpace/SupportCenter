...
inline UINT
vncEncodeZlib::EncodeRect(BYTE *source,BYTE *source2, boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/, BYTE *dest, const rfb::Rect &rect)
{
	int  totalSize = 0;
	int  partialSize = 0;
	int  maxLines;
	...
	while ( linesRemaining > 0 ) {
		...
		partialSize = EncodeOneRect( source,source2, dest, partialRect, outstream );
			...
			( partialSize > 0 ))
		{
			...
			try
			{
				outstream->Send2Queue( (char *)dest, partialSize );
			}
...
// Encode the rectangle using zlib compression
inline UINT
vncEncodeZlib::EncodeOneRect(BYTE *source,BYTE *source2, BYTE *dest, const RECT &rect,boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
		...
		if (m_queueEnable && source2 && dataSize>Firstrun)
			{
				...
				AddToQueu(dest,sz_rfbFramebufferUpdateRectHeader +rawDataSize,outstream,0);
				return 0;
			}
	...
	switch (SoMoMu)
	{
		case SOLID_COLOR:
			{
				...
				if (m_queueEnable)
				{
				AddToQueu(dest,sz_rfbFramebufferUpdateRectHeader +newsize,outstream,0);
				return 0;
				}
		...
		case MONO_COLOR:
			{
				...
				AddToQueu(dest,sz_rfbFramebufferUpdateRectHeader +newsize,outstream,1);
				return 0;
				}
				break;
		...
		case PURE_ZLIB:
			{
				...
				surh->encoding = Swap32IfLE(rfbEncodingRaw);
				memcpy(dest+sz_rfbFramebufferUpdateRectHeader,m_buffer,rawDataSize);
				AddToQueu(dest,sz_rfbFramebufferUpdateRectHeader +rawDataSize,outstream,1);
				return 0;
				}
		...
		if ( compStreamInited == false )
		{
			...
			if ( deflateResult != Z_OK )
			{
				...
				Log.Add(_ERROR_, "deflateInit2 returned error:%d:%s", deflateResult, compStream.msg);
				return vncEncoder::EncodeRect(source, dest, rect);
			}
		...
		if ( deflateResult != Z_OK )
		{
			...
			Log.Add(_ERROR_, "deflate returned error:%d:%s", deflateResult, compStream.msg);
			return vncEncoder::EncodeRect(source, dest, rect);
		}
...
void
vncEncodeZlib::AddToQueu(BYTE *source,int sizerect,boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/,int updatetype)
{
	if (m_Queuelen+sizerect>(MaxQueuebufflen)) SendZlibrects(outstream);
	memcpy(m_Queuebuffer+m_Queuelen,source,sizerect);
	m_Queuelen+=sizerect;
	m_nNbRects++;
	if (updatetype==1) must_be_zipped=true;
	if (m_nNbRects>50) SendZlibrects(outstream);
}

void
vncEncodeZlib::SendZlibrects(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
	...
	if (m_nNbRects<3 && !must_be_zipped) 
	{
		try
		{
			outstream->Send2Queue( (char *)m_Queuebuffer, m_Queuelen); // 1 Small update
		}
		catch(CStreamException&)
		{
		}
	...
	if (nRet != 0)
	{
		...
		Log.Add(_ERROR_, "compression error");
		return ;
	}
	...
	try
	{
		outstream->Send2Queue((char *)&CacheRectsHeader, sizeof(CacheRectsHeader));
		outstream->Send2Queue((char *)&CacheZipHeader, sizeof(CacheZipHeader));
		outstream->Send2Queue((char *)m_QueueCompressedbuffer, maxCompSize);
	}
	catch(CStreamException&)
	{
	}
...
void
vncEncodeZlib::LastRect(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
	SendZlibrects(outstream);
}