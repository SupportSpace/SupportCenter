...
inline UINT
vncEncodeUltra::EncodeRect(BYTE *source, boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/, BYTE *dest, const rfb::Rect &rect)
{
	...
	while ( linesRemaining > 0 ) {
		...
		partialSize = EncodeOneRect( source,dest, partialRect,outstream );
...
inline UINT
vncEncodeUltra::EncodeOneRect(BYTE *source, BYTE *dest, const RECT &rect, boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/)
{
...
void
vncEncodeUltra::AddToQueu(BYTE *source,int sizerect,boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/,int updatetype)
{
	if (m_Queuelen+sizerect>(MaxQueuebufflen)) SendUltrarects(outstream);
	memcpy(m_Queuebuffer+m_Queuelen,source,sizerect);
	m_Queuelen+=sizerect;
	m_nNbRects++;
	if (updatetype==1) must_be_zipped=true;
	if (m_nNbRects>0) SendUltrarects(outstream);
}

void
vncEncodeUltra::AddToQueu2(BYTE *source,int sizerect,boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/,int updatetype)
{
	...
	if (out_len>rawDataSize)
				{
					try
					{
						outstream->Send2Queue( (char *)source, sizerect); // 1 Small update
					}
					catch(CStreamException&)
					{
					}
	...
	try
	{
		outstream->Send2Queue((char *)m_QueueCompressedbuffer, out_len+sz_rfbFramebufferUpdateRectHeader+sz_rfbZlibHeader);
	}
	catch(CStreamException&)
	{
	}
...
void
vncEncodeUltra::SendUltrarects(boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/)
{
	...
	try
	{
		...
		outstream->Send2Queue((char *)m_QueueCompressedbuffer, out_len);
	}
	catch(CStreamException&)
	{
	}
...
void
vncEncodeUltra::LastRect(boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/)
{
	SendUltrarects(outstream);
}