...
namespace rfb {
	...
	struct Rect {
		inline Rect() {}
		inline Rect(Point tl_, Point br_) : tl(tl_), br(br_) {}
		inline Rect(int x1, int y1, int x2, int y2) : tl(x1, y1), br(x2, y2) {}
#ifdef WIN32
		inline Rect(const RECT &r) : tl(r.left, r.top), br(r.right, r.bottom) {}
#endif
		...
		inline unsigned int area() const {return is_empty() ? 0 : width()*height();}
