...
UINT
vncEncodeTight::EncodeRect(BYTE *source, boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/, BYTE *dest,
						   const RECT &rect)
{
	...
	if (!m_use_lastrect || w * h < MIN_SPLIT_RECT_SIZE)
		return EncodeRectSimple(source, outstream, dest, rect);
	...
	for (dy = y; dy < y + h; dy += MAX_SPLIT_TILE_SIZE) {
		...
		if (dy - y >= nMaxRows) {
			...
			int size = EncodeRectSimple(source, outstream, dest, upperRect);
			try
			{
				//outConn->SendExactQueue((char *)dest, size);
				outstream->Send2Queue((char*)dest,size);
			}
		...
		for (dx = x; dx < x + w; dx += MAX_SPLIT_TILE_SIZE) {
			...
			if (CheckSolidTile(source, dx, dy, dw, dh, &colorValue, FALSE)) {
				...
				// Receive dimensions of solid-color area.
				...
				for (int i = 0; i < 4; i++) {
					if (i == 2) {
						...
						try
						{
							outstream->Send2Queue((char *)m_hdrBuffer, m_hdrBufferBytes);
							outstream->Send2Queue((char *)dest, size);
						}
						catch(CStreamException&)
						{
					...
					int size = EncodeRect(source, outstream, dest, rects[i]);
					try
					{
						outstream->Send2Queue((char *)dest, size);
					}
...
UINT
vncEncodeTight::EncodeRectSimple(BYTE *source, boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/, BYTE *dest,
								 const RECT &rect)
{
	...
	if (w > maxRectWidth || w * h > maxRectSize) {
		...
		for (dy = 0; dy < h; dy += subrectMaxHeight) {
			for (dx = 0; dx < w; dx += maxRectWidth) {
				...
				partialSize = EncodeSubrect(source, outstream, dest,
											x+dx, y+dy, rw, rh);
		...
		partialSize = EncodeSubrect(source, outstream, dest, x, y, w, h);
	}
...
UINT
vncEncodeTight::EncodeSubrect(BYTE *source, boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/, BYTE *dest,
							 ...
							 int zlibLevel, int zlibStrategy)
{
	...
	if (!m_zsActive[streamId]) {
		...
		if (err != Z_OK) 
		{
			...
			//			 err, pz->msg);
			Log.Add(_ERROR_, "deflateInit2 returned error:%d:%s", err, pz->msg);
			return -1;
		}
	...
	if (zlibLevel != m_zsLevel[streamId]) 
	{
		...
		if (err != Z_OK) 
		{
			...
			//			 err, pz->msg);
			Log.Add(_ERROR_, "deflateParams returned error:%d:%s", err, pz->msg);
			return -1;
		}
		m_zsLevel[streamId] = zlibLevel;
	}
		 ...
		 pz->avail_in != 0 || pz->avail_out == 0 ) 
	{
		...
		Log.Add(_ERROR_, "deflate() call failed.");
		return -1;
	}
