...
void TextChat::ProcessTextChatMsg(int nTO)
{
	...
	try
	{
		m_pCC->m_stream->Get(((char *) &tcm) + nTO, sz_rfbTextChatMsg - nTO);
	}
	...
	else
	{
		...
		else
		{
			...
			try
			{
				m_pCC->m_stream->Get(m_szRemoteText, len);
			}
			catch(CStreamException&)
			{
			}
...
void TextChat::SendTextChatRequest(int nMsg)
{
	...
	try
	{
		m_pCC->m_stream->Put((char *)&tcm, sz_rfbTextChatMsg);
		//m_pCC->m_stream->PutMsg((char *)&tcm, sz_rfbTextChatMsg, rfbTextChat);
	}
	catch(CStreamException&)
	{
	}
...
void TextChat::SendLocalText(void)
{
	...
	try
	{
		m_pCC->m_stream->Put((char *)&tcm, sz_rfbTextChatMsg);
		//m_pCC->m_stream->PutMsg((char *)&tcm, sz_rfbTextChatMsg, rfbTextChat);
		m_pCC->m_stream->Put((char *)m_szLocalText, strlen(m_szLocalText));
	}
	catch(CStreamException&)
	{
	}
