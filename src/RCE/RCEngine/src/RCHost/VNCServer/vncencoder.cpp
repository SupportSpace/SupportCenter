...
vncEncoder::Translate(BYTE *source, BYTE *dest, const rfb::Rect &rect)
{
	...
	TRY_CATCH
		...
		return;
	CATCH_LOG()
		Log.Add(_ERROR_,_T("vncEncoder::Translate fail info: rect tl(%d; %d),br(%d; %d)"), rect.tl.x, rect.tl.y, rect.br.x, rect.br.y);
		Log.Add(_ERROR_,_T("vncEncoder::Translate fail info: m_bytesPerRow = %d"),m_bytesPerRow);
		Log.Add(_ERROR_,_T("vncEncoder::Translate fail info: source = %d"),source);
}
...
inline UINT
vncEncoder::EncodeRect(BYTE *source, BYTE *source2, boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/, BYTE *dest, const rfb::Rect &rect)
{
...
vncEncoder::GetRemotePalette(RGBQUAD *quadlist, UINT ncolours)
{
	...
	if (m_localpalette == NULL)
	{
		...
		Log.Add(_ERROR_, "failed to obtain colour map data!");
		return FALSE;
	}
...
vncEncoder::SetTranslateFunction()
{
		...
		(m_transformat.bitsPerPixel != 32))
    {
		...
		//	);
		Log.Add(_WARNING_, "only 8, 16 or 32 bits supported remotely - %d requested", m_transformat.bitsPerPixel);
		return FALSE;
    }
		...
		(m_localformat.bitsPerPixel != 32))
    {
		...
		//	);
		Log.Add(_WARNING_,"only 8, 16 or 32 bits supported locally - %d in use", m_localformat.bitsPerPixel);
		return FALSE;
    }
	...
	if (!m_transformat.trueColour && (m_transformat.bitsPerPixel != 8))
	{
		...
		Log.Add(_WARNING_, "only 8-bit palette format supported remotely");
		return FALSE;
	}
	...
	if (!m_localformat.trueColour && (m_localformat.bitsPerPixel != 8))
	{
		...
		Log.Add(_WARNING_, "only 8-bit palette format supported locally");
		return FALSE;
	}
    ...
    if (!m_transformat.trueColour)
	{
		...
		else
		{
			...
			Log.Add(_ERROR_, "unknown local pixel format in use!");
			return FALSE;
		}
	}
...
inline void
vncEncoder::LastRect(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
