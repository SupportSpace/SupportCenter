...
vncBuffer::CheckBuffer()
{
	...
	if ((m_backbuffsize != m_desktop->ScreenBuffSize()) || !m_freemainbuff)
	{
		...
		// Modif rdv@2002 - v1.1.x - Videodriver
			...
			if (m_mainbuff) {
				...
				if ((m_mainbuff = new BYTE [m_desktop->ScreenBuffSize()]) == NULL)
				{
					...
					Log.Add(_ERROR_, "unable to allocate main buffer[%d]", m_desktop->ScreenBuffSize());
					return FALSE;
				}
		...
		if ((m_backbuff = new BYTE [m_desktop->ScreenBuffSize()]) == NULL)
		{
			...
			Log.Add(_ERROR_, "unable to allocate back buffer[%d]", m_desktop->ScreenBuffSize());
			return FALSE;
		}
		...
		if (m_use_cache)
		{
			if ((m_cachebuff = new BYTE [m_desktop->ScreenBuffSize()]) == NULL)
			{
				...
				Log.Add(_ERROR_, "unable to allocate cache buffer[%d]", m_desktop->ScreenBuffSize());
				return FALSE;
			}
			ClearCache();
		}
		...
		if (m_nScale > 1) 
		{
			...
			if ((m_ScaledBuff = new BYTE [m_desktop->ScreenBuffSize()]) == NULL)
			{
				...
				Log.Add(_ERROR_, "unable to allocate scaled buffer[%d]", m_desktop->ScreenBuffSize());
				return FALSE;
			}
	...
	if (m_scrinfo.format.trueColour)
	{
		...
		for (int y = ScaledRect.tl.y; y < ScaledRect.br.y; y++)
		{
			...
			for (int x = 0; x < (ScaledRect.br.x - ScaledRect.tl.x); x++)
			{
				...
				// Receive the medium R,G,B values for the sqare
...
vncBuffer::ClearBack()
{
	...
	if (m_desktop->IsCaptureAlphaBlending())
		m_desktop->ClipHiddenWindows(GetSize(),m_backbuff,m_backbuffsize);
}
...
vncBuffer::FastCheckMainbuffer() {
	...
	if (tmp && (m_mainbuff != tmp))
		{
//			boost::recursive_mutex::scoped_lock l(m_desktop->m_videodriver_lock2);
