...
{
private:
	boost::recursive_mutex m_setEncodingLock;
public:
	...
	inline bool HasUpdateLock()
	{
		return	m_buffer != NULL 
				&& 
				m_buffer->m_desktop != NULL;
	}
	...
	inline UINT EncodeRect(const rfb::Rect &rect, /*VSocket *outconn*/ boost::shared_ptr<CAbstractStream> outstream);
	...
	inline BOOL SendCursorShape(boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/);
	inline BOOL SendEmptyCursorShape(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/);
	...
	// QUEUE ZLIBXOR
	inline void LastRect(boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/);
	// Modif cs@2005
//#ifdef DSHOW
...
vncEncodeMgr::GetPalette(RGBQUAD *quadlist, UINT ncolours)
{
	...
	if (m_encoder == NULL)
	{
		...
		Log.Add(_ERROR_, "GetPalette called but no encoder set");
		return FALSE;
	}
...
vncEncodeMgr::SetEncoding(CARD32 encoding,BOOL reinitialize)
{
	...
	/// This is needed to prevent simulation access from vncClienThread and vncDesktopThread
	boost::recursive_mutex::scoped_lock l(m_setEncodingLock);
	...
	case rfbEncodingUltra:
		...
		{
			Log.Add(_MESSAGE_,_T("Creating new vncEncodeUltra"));
	...
	case rfbEncodingZlib:
		...
		{
			Log.Add(_MESSAGE_,_T("Creating new vncEncodeZlib"));
	...
	case rfbEncodingZlibHex:
		...
		{
			Log.Add(_MESSAGE_,_T("Creating new vncEncodeZlibHex"));
	...
	case rfbEncodingTight:
		...
		{
			Log.Add(_MESSAGE_,_T("Creating new vncEncodeTight"));
	...
	default:
		...
		Log.Add(_ERROR_, "unknown encoder requested");
		return FALSE;
	}
	...
	if (m_clientfmtset)
		if (!m_encoder->SetRemoteFormat(m_clientformat))
		{
			...
			Log.Add(_ERROR_,_T("Client pixel format is not supported bpp(%d) redMax(%d) greeMax(%d) blueMax(%d)"),m_clientformat.bitsPerPixel, m_clientformat.redMax, m_clientformat.greenMax, m_clientformat.blueMax);
			return FALSE;
		}
...
inline UINT
vncEncodeMgr::EncodeRect(const rfb::Rect &rect, boost::shared_ptr<CAbstractStream> outstream /*VSocket *outconn*/)
{
	...
	if (!m_buffer->m_backbuff)
	{
		...
		Log.Add(_ERROR_, "no client back-buffer available in EncodeRect");
		return 0;
	}
	if (zlib_encoder_in_use)
	{
		if (m_use_xor)
		{
		return m_encoder->EncodeRect(m_buffer->m_backbuff, m_buffer->m_cachebuff, outstream ,m_clientbuff, rect);
		}
		else return m_encoder->EncodeRect(m_buffer->m_backbuff, NULL, outstream ,m_clientbuff, rect);
	}
	if (ultra_encoder_in_use)
	{
		return m_encoder->EncodeRect(m_buffer->m_backbuff, outstream ,m_clientbuff, rect);
	}
	...
	if (tight_encoder_in_use || zlibhex_encoder_in_use  || ultra_encoder_in_use)
	{
		...
		TRect.top = rect.tl.y;
		TRect.bottom = rect.br.y;
		return m_encoder->EncodeRect(m_buffer->m_backbuff, outstream, m_clientbuff, TRect); // sf@2002 - For Tight...
	}
...
inline BOOL
vncEncodeMgr::SendCursorShape(boost::shared_ptr<CAbstractStream> outstream/*VSocket *outConn*/) {
	return m_encoder->SendCursorShape(outstream, m_buffer->m_desktop);
}

inline BOOL
vncEncodeMgr::SendEmptyCursorShape(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/) {
	return m_encoder->SendEmptyCursorShape(outstream);
}
...
inline void
vncEncodeMgr::LastRect(boost::shared_ptr<CAbstractStream> outstream /*VSocket *outConn*/)
{
	m_encoder->LastRect(outstream);
}
// Modif cs@2005
//#ifdef DSHOW
inline BOOL
vncEncodeMgr::ResetZRLEEncoding(void)
{
	if (NULL != zrleEncoder)
	{
		...
		if (m_clientfmtset)
		{
			...
			{
				//vnclog.Print(LL_INTERR, VNCLOG("client pixel format is not supported\n"));
				Log.Add(_ERROR_, "client pixel format is not supported");
...
inline boost::recursive_mutex& vncEncodeMgr::GetUpdateLock()
{
	return m_buffer->m_desktop->GetUpdateLock();
};

inline BOOL vncEncodeMgr::CheckBuffer()
{
	// Receive the screen format, in case it has changed
	...
	if (m_clientbuffsize != clientbuffsize)
	{
	    ...
	    {		
			...
			Log.Add(_ERROR_, "unable to allocate client buffer[%u]", clientbuffsize);
			return FALSE;
	    }
