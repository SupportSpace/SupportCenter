...
void ClientConnection::ReadCursorShape(rfbFramebufferUpdateRectHeader *pfburh) 
{
	...
	boost::recursive_mutex::scoped_lock l(m_cursorMutex);
...
void ClientConnection::SoftCursorSaveArea() {
	...
	if (!BitBlt(m_hSavedAreaDC, 0, 0, w, h, m_hBitmapDC, x, y, SRCCOPY)) 
	{
		...
		Log.Add(_ERROR_, "Error saving screen under cursor");
	}
}
...
void ClientConnection::SoftCursorRestoreArea() 
{
	...
	if (!BitBlt(m_hBitmapDC, x, y, w, h, m_hSavedAreaDC, 0, 0, SRCCOPY)) 
	{
		...
		Log.Add(_ERROR_, "Error restoring screen under cursor");
	}
...
void ClientConnection::SoftCursorDraw() {
	...
	if (SCALE_MODE == m_opts.m_displayMode ||
		(FULLSCREEN_MODE == m_opts.m_displayMode && (m_opts.m_scale_den / m_opts.m_dx > 1.0 || m_opts.m_scale_den / m_opts.m_dy > 1.0)))
	{
		/// To avoid flickering we cant invalidate small pieces of screen
