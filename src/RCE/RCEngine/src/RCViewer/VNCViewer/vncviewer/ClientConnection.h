...
#include "fullscreen_routine.h"
#include "Streaming/CAsyncSendStream.h"
...
{
	int m_length;
	std::queue<int> m_sequence;
	int m_sum;
public:
	/// ctor
	CIntegrator(const int length)
		:	m_length(length), 
			m_sum(0)
	{
	...
	/// Inserts next value into filter, returns average value
	inline int Next(const int value)
	{
		m_sum -= m_sequence.front();
		m_sum += value;
		m_sequence.pop();
		m_sequence.push(value);
		return m_sum / m_length;
	}

};

/// Default length of integrating filter
#define DEF_INTEG_FILTER_LENGTH 3
...
class ClientConnection
{
	friend class CRCViewer;
private:
	/// Integrating filter
	CIntegrator m_integrator;

	// Timers -----------------------------------------------------------------
	friend class CScopedTracker<UINT_PTR>;
	/// Destroys internal timer (needed to use CScopedTracker for internal timers)
	void KillInternalTimer(UINT_PTR timerId);

	/// Alive timer
	CScopedTracker<UINT_PTR> m_aliveTimer;
	#define ALIVE_MESSAGES_INTERVAL 15000
	/// true if alive message is needed
	bool m_sentRecently, m_recvRecently;

	/// Stream flush timer
	CScopedTracker<UINT_PTR> m_streamTimer;
	#define FLUSH_STREAM_INTERVAL 200
	...
	{
		if (NULL != GetSrvCommunicator())
		{
			SToggleTaskbar msg;
			msg.pid = GetCurrentProcessId();
			msg.show = !fullScreen;
			GetSrvCommunicator()->SendMsg(SRVCOMM_TOGGLE_TASKBAR, reinterpret_cast<char*>(&msg), sizeof(msg));
		}
		else
		{
			m_FullScreenToggle.ToggleFullScreen( fullScreen );
		}
	}	

	/// Stream for async sending
	CAsyncSendStream m_asyncStream;
	/// True only after initial stuff was sent
	bool m_inited;
	/// thread object for ClientConnection instance
	std::auto_ptr<boost::thread> m_thread;
	int m_newColors;
	bool m_pendingColorsRequest;
	/// send initial stuff message
	static unsigned int m_msgSendInitialStuff;
	/// send retry stuff (if protocol was broken)
	static unsigned int m_msgSendRetryStuff;
	/// Send stop session message
	static unsigned int m_msgSendStopSession;
	CHiderTaskBar m_FullScreenToggle;
	/// Scrollbars
	HWND m_horScrollBar, m_vertScrollBar, m_vertScrollBarParent;
	/// clipboard fix
	tstring m_recentClipboardText;
protected:
...
public:
	...
	ClientConnection(VNCviewerApp *pApp, boost::shared_ptr<CAbstractStream> stream, CRCViewer*,HWND);
	virtual ~ClientConnection();
	void Run();
	void KillThread();
	HWND GetMainWindow();
	HWND GetOriginalWindow();
	...
	void ChangeDisplayMode( EDisplayMode );
	void TurnOffFullScreen();

private:

	EDisplayMode m_prevDisplayMode;

	/// Speed measured event handler
	inline void OnSpeedMeasured(const unsigned int speed);
	...
	static LRESULT CALLBACK KeyWndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam);
	...
	rfbPixelFormat SelectPixelFormat(int ColorsCount);
	void SetupPixelFormat();
	void SetFormatAndEncodings(bool sendColorsFormat = true);
	...
	inline void ReadExact(char *buf, int bytes);
	void ReadString(char *buf, int length);
	inline void WriteExact(char *buf, int bytes);
	inline void WriteExact(char *buf, int bytes, CARD8 msgType); //sf@2002 - DSM Plugin
	...
	void run_undetached();
	void DoMainLoop();
	/// Try to restart session. If failed, exceptioni is thrown
	void TryRestartSession();
	bool m_bKillThread;
	...
	boost::recursive_mutex m_zlibBufferMutex;
	boost::recursive_mutex m_bitmapdcMutex;
	boost::recursive_mutex m_clipMutex;
	boost::recursive_mutex m_writeMutex;
	boost::recursive_mutex m_sockMutex;
	boost::recursive_mutex m_cursorMutex;
	boost::recursive_mutex m_readMutex;
	...
	boost::recursive_mutex 	m_NetRectBufferMutex;
	boost::recursive_mutex 	m_ZRLENetRectBufferMutex;
	boost::recursive_mutex 	m_ZipBufferMutex;
	boost::recursive_mutex 	m_FileZipBufferMutex;
	boost::recursive_mutex 	m_FileChunkBufferMutex;
	boost::recursive_mutex 	m_ZlibBufferMutex;
	...
	rfbPixelFormat m_myFormat;
	// protocol version in use.
	int m_majorVersion, m_minorVersion;
	bool m_threadStarted, m_running, m_restarting;
...
inline void ClientConnection::WriteExact(char *buf, int bytes, CARD8 msgType)
{
