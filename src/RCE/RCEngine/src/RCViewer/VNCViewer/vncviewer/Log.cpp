...
const int ColdLog::ToDebug   =  1;
const int ColdLog::ToFile    =  2;
const int ColdLog::ToConsole =  4;
...
void ColdLog::SetLevel(int level) {
    m_level = level;
}

void ColdLog::SetFile(LPTSTR filename, bool append) 
{
...
void ColdLog::ReallyPrint(LPTSTR format, va_list ap) 
{
	...
	{
        DWORD byteswritten;
        WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE), line, static_cast<DWORD>(_tcslen(line)*sizeof(TCHAR)), &byteswritten, NULL); 
    }

    if (m_tofile && (hlogfile != NULL)) 
	{
        DWORD byteswritten;
        WriteFile(hlogfile, line, static_cast<DWORD>(_tcslen(line)*sizeof(TCHAR)), &byteswritten, NULL); 
