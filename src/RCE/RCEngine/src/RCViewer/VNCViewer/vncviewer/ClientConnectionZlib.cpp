...
void ClientConnection::ReadZlibRect(rfbFramebufferUpdateRectHeader *pfburh,int XOR) {
	...
	if ( m_decompStreamInited == false ) {
		...
		if ( inflateResult != Z_OK ) 
		{
			...
			Log.Add(_ERROR_, "zlib inflateInit error: %d", inflateResult);
			return;
		}
	...
	if ( inflateResult < 0 ) 
	{
		...
		Log.Add(_ERROR_, "zlib inflate error: %d", inflateResult);
		return;
	}
...
void ClientConnection::CheckZlibBufferSize(int bufsize)
{
	...
	boost::recursive_mutex::scoped_lock l(m_zlibBufferMutex);
