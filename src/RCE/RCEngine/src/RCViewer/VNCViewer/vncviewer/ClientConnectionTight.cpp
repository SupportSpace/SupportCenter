...
void ClientConnection::ReadTightRect(rfbFramebufferUpdateRectHeader *pfburh)
{
    ...
    if ((comp_ctl & 1) && m_tightZlibStreamActive[i]) 
	{
      ...
      if (err != Z_OK) 
	  {
        if (m_tightZlibStream[i].msg != NULL) 
		{
		  ...
		  Log.Add(_ERROR_, "zlib inflateEnd() error: %s", m_tightZlibStream[i].msg);
        } 
		else 
		{
          //vnclog.Print(0, _T("zlib inflateEnd() error: %d\n"), err);
		  Log.Add(_ERROR_, "zlib inflateEnd() error: %d", err);
        }
  ...
  if (comp_ctl >= rfbTightMaxSubencoding) 
  {
	...
	Log.Add(_ERROR_, "Tight encoding: bad subencoding value received.");
    return;
  }
  ...
  if ((comp_ctl & rfbTightExplicitFilter) != 0) 
  {
    ...
    default:
	  ...
	  Log.Add(_ERROR_, "Tight encoding: unknown filter code received.");
      return;
    }
  } 
  else 
  ...
  if (bitsPixel == 0) 
  {
	...
	Log.Add(_ERROR_, "Tight encoding: error receiving palette.");
    return;
  }
  ...
  if (compressedLen <= 0) 
  {
	...
	Log.Add(_ERROR_, "Tight encoding: bad data received from server.");
    return;
  }
  ...
  if (!m_tightZlibStreamActive[stream_id]) 
  {
    ...
    if (err != Z_OK) 
	{
      if (zs->msg != NULL) 
	  {
		...
		Log.Add(_ERROR_, "zlib inflateInit() error: %s.", zs->msg);
      } 
	  else 
	  {
        //vnclog.Print(0, _T("zlib inflateInit() error: %d."), err);
		Log.Add(_ERROR_, "zlib inflateInit() error: %d.", err);
      }
      return;
    }
    ...
    do 
	{
      ...
      if (err != Z_OK && err != Z_STREAM_END) 
	  {
        if (zs->msg != NULL) 
		{
		  ...
		  Log.Add(_ERROR_, "zlib inflate() error: %s.", zs->msg);
        } 
		else 
		{
          //vnclog.Print(0, _T("zlib inflate() error: %d.\n"), err);
		  Log.Add(_ERROR_, "zlib inflate() error: %d.", err);
        }
        return;
      }
  ...
  if (rowsProcessed != pfburh->r.h) 
  {
	...
	Log.Add(_ERROR_, "Tight encoding: wrong number of scan lines.");
  }
}
...
void ClientConnection::DecompressJpegRect(int x, int y, int w, int h)
{
  ...
  if (compressedLen <= 0) 
  {
	...
	Log.Add(_ERROR_, "Incorrect data received from the server.");
    return;
  }
      ...
      cinfo.output_components != 3) 
  {
	...
	Log.Add(_ERROR_,"Tight Encoding: Wrong JPEG data received.");
