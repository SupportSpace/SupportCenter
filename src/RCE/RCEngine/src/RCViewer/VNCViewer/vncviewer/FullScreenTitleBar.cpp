...
#define FSTB_CLASS_NAME _T("FSTITLEBAR")
...
{
private:
	std::map<const HWND,CTitleBar*> m_titleBarsMap;
public:
	/// Add titlebar to map
	/// @param titleBar new titlebar
	void Add(CTitleBar* titleBar)
	{
	TRY_CATCH
		m_titleBarsMap[titleBar->m_hWnd] = titleBar;
	CATCH_THROW("CTitleBars::Add")
	}

	/// Remove titlebar from map
	/// @param titleBar titlebar to remove
	void Remove(CTitleBar* titleBar)
	{
	TRY_CATCH
		m_titleBarsMap.erase(titleBar->m_hWnd);
	CATCH_THROW("CTitleBars::Remove")
	}

	/// Returns titleBar by hWnd
	/// @param hWnd title bars' window handle
	/// @return found CTitleBar* or NULL if not found
	CTitleBar* Get(const HWND hWnd) const
	{
	TRY_CATCH
		std::map<const HWND,CTitleBar*>::const_iterator res = m_titleBarsMap.find(hWnd);
		if (res == m_titleBarsMap.end()) return NULL;
		return (*res).second;
	CATCH_THROW("CTitleBars::Get")
	}
};

/// Should be used to CTitleBars as single instance
#define TITLEBARS_INSTANCE Loki::SingletonHolder<CTitleBars, Loki::CreateUsingNew, Loki::NoDestroy, Loki::ClassLevelLockable>::Instance()
...
TRY_CATCH
	...
	Parent=NULL;
	this->Init();
CATCH_THROW("CTitleBar::CTitleBar")
}

CTitleBar::CTitleBar(HINSTANCE hInst, HWND ParentWindow)
{
TRY_CATCH
	hInstance=::GetCurrentModule();//hInst;
	Parent=ParentWindow;
	this->Init();
CATCH_THROW("CTitleBar::CTitleBar")
}
...
TRY_CATCH
	...
	TITLEBARS_INSTANCE.Remove(this);
	UnregisterClass(FSTB_CLASS_NAME, GetModuleHandle(NULL));
CATCH_LOG("CTitleBar::~CTitleBar");
}
...
TRY_CATCH
	...
	if(Parent!=NULL&&hInstance!=NULL)
	{
		///TitleBarThis=this;
		this->CreateDisplay();
		TITLEBARS_INSTANCE.Add(this);
	}
CATCH_THROW("CTitleBar::Init")
}
...
TRY_CATCH
	...
	wndclass.lpszClassName	= FSTB_CLASS_NAME;
...
CATCH_THROW("CTitleBar::CreateDisplay")
}
...
TRY_CATCH
	...
	CTitleBar* TitleBarThis = TITLEBARS_INSTANCE.Get(hwnd);
	//if (!TitleBarThis) Log.Add(_WARNING_,"TitleBarThis == NULL");
		...
		//Menu part starts here
		{
			...
			if(IDNum>=tbWMCOMMANDIDStart&&IDNum<tbWMCOMMANDIDEnd) //The ID is in range for a menuclick
			{
				...
				else //Handle close, minimize and maximize
				{
					...
					//Receive the real number of entries (exluding seperators)
	...
	case WM_TIMER:
		{
			...
			if(TimerID==tbScrollTimerID)
			{
				...
				SetWindowPos(TitleBarThis->m_hWnd, HWND_TOPMOST,  lpRect.left, lpRect.top, lpRect.right-lpRect.left, lpRect.bottom-lpRect.top, 0);
...
CATCH_THROW("CTitleBar::LoadPictures")
}
...
CATCH_THROW("CTitleBar::FreePictures")
}
...
TRY_CATCH
	...
	IntAutoHideCounter = -10 * tbAutoScrollTime; //0;
	if(Show==TRUE)
	{
		if(tbScrollWindow==TRUE)
		{
...
CATCH_THROW("CTitleBar::DisplayWindow")
}
