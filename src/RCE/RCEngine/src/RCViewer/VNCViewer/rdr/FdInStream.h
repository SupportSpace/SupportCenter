...
namespace rdr {
  ...
  public:
	...
	FdInStream(CAbstractStream* stream_ );
    FdInStream(int fd, int timeout=0, int bufSize=0);
    FdInStream(int fd, void (*blockCallback)(void*), void* blockCallbackArg=0,int bufSize=0);
    virtual ~FdInStream();
  ...
  private:
    ...
    inline int readWithTimeoutOrCallback(void* buf, int len);
