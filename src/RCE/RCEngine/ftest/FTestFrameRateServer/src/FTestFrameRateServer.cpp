...
#include "stdafx.h"
#include "FTestFrameRateServer.h"
...
TRY_CATCH
	...
	LoadString(hInstance, IDC_FRAMERATETESTSERVER, szWindowClass, MAX_LOADSTRING);
	...
	FRAMERATESERVER_INSTANCE.Start();
...
TRY_CATCH
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_FRAMERATETESTSERVER));
...
TRY_CATCH
	...
	case WM_PAINT:
		hdc = BeginPaint(hWnd, &ps);
		{
			...
			CCritSection section(FRAMERATESERVER_INSTANCE.GetSection());
			// Get frame
			SPFrame frame = FRAMERATESERVER_INSTANCE.GetCurrentFrame();
			// Draw frame
			if(frame.get())
				frame->Draw();
		}
...
TRY_CATCH
	...
	while(*(++pArgs))
	{
		...
		if( !paramName.compare(_T("-F=")) )
			...
			SETTINGS_INSTANCE.SetRepeats(atoi(param.c_str()));

		else if( !paramName.compare(_T("-P=")) )
			SETTINGS_INSTANCE.SetLocalPort(atoi(param.c_str()));

		else if( !paramName.compare(_T("-PO")) )
			SETTINGS_INSTANCE.SetServerPlayOnly(true);
