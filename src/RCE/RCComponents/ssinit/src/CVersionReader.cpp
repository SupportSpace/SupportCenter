...
tstring CVersionReader::GetProductCodeByUpgradeCode(const tstring& upgradeCode)
{
	...
	{
		SetLastError(res);
		throw tstring(_T("Failed to MsiEnumRelatedProducts")); //TODO: add winerror description
	}
	productCode = code;
	if (ERROR_SUCCESS == MsiEnumRelatedProducts(upgradeCode.c_str(), 0 /*reserved*/, 1 /*next product*/, code))
	{
		throw tstring(_T("There's more than one product installed"));
	}
	return productCode;
}
