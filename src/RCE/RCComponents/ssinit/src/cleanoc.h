...
void GetCodeBaseUrlFromCLSIDstr(char *url,const size_t urlLength,char *clsid,const size_t clsidLength)
{
	url[0]=_T('\0');
	_strupr_s(clsid,clsidLength);
	DWORD res;
	HMODULE hModule;
	if((hModule=::LoadLibraryA("occache.dll"))==NULL)
		throw std::exception("occache.dll loading failed");
	FINDFIRSTCONTROL findFirstControl=(FINDFIRSTCONTROL)::GetProcAddress(hModule,axcFINDFIRSTCONTROL);
	if(!findFirstControl)
		throw std::exception("FindFirstControl() call address obtaining failed");
	FINDNEXTCONTROL findNextControl=(FINDNEXTCONTROL)::GetProcAddress(hModule,axcFINDNEXTCONTROL);
	if(!findNextControl)
		throw std::exception("FindNextControl() call address obtaining failed");
	FINDCONTROLCLOSE findControlClose=(FINDCONTROLCLOSE)::GetProcAddress(hModule,axcFINDCONTROLCLOSE);
	if(!findControlClose)
		throw std::exception("FindControlClose() call address obtaining failed");
	RELEASECONTROLHANDLE releaseControlHandle=(RELEASECONTROLHANDLE)::GetProcAddress(hModule,axcRELEASECONTROLHANDLE);
	if(!releaseControlHandle)
		throw std::exception("ReleaseControlHandle() call address obtaining failed");
	GETCONTROLINFO getControlInfo=(GETCONTROLINFO)::GetProcAddress(hModule,axcGETCONTROLINFO);
	if(!getControlInfo)
		throw std::exception("GetControlInfo() call address obtaining failed");
	HANDLE find,control;
	if((res=findFirstControl(find,control,NULL))==ERROR_SUCCESS)
	{
		do
		{
			char s[8192];
			if(getControlInfo(control,GCI_CLSID,NULL,(LPTSTR)s,sizeof(s)/sizeof(*s)))
			{
				_strupr_s(s,sizeof(s)/sizeof(*s));
				if(strcmp(s,clsid)==0)
				{
					if(!getControlInfo(control,GCI_CODEBASE,NULL,(LPTSTR)s,sizeof(s)/sizeof(*s)))
						throw std::exception("CODEBBASE obtaining failed");
					strcpy_s(url,urlLength,s);
				}
			}
			releaseControlHandle(control);
		}
		while(findNextControl(find,control)==ERROR_SUCCESS);
		findControlClose(find);
	}
	::FreeLibrary(hModule);
}
