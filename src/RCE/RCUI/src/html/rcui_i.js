...
var hostObject=null;
/// the installer activeX object (CRCInstallerAXCtrl)
var	installerObject=null;

/// set of services
var servicesSet=new Object();
servicesSet['RCViewer']=new CServiceInfo("RCUI.RCViewerAXCtrl",0,"RemoteControl","","","2.0");
servicesSet['RCHost']=new CServiceInfo("RCUI.RCHostAXCtrl",1,"RemoteControl","","","2.0");
servicesSet['RCPlayback']=new CServiceInfo("RCUI.RCViewerAXCtrl",2,"RemoteControl","","","2.0");
servicesSet['RCFileAccessClient']=new CServiceInfo("FTUI.IFileAccessClientX",3,"FileAccess","","","2.0");
servicesSet['RCFileAccessHost']=new CServiceInfo("FTUI.IFileAccessHost",4,"FileAccess","","","2.0");
servicesSet['RCInstaller']=new CServiceInfo("RCInstaller.RCInstallerAXCtrl",5,"RCInstaller","","","1.0");

/// CServiceInfo class - information of available service
function CServiceInfo(name,type,fname,guid,version,reqVersion)
{
	///service name
	this.m_name=name;
	/// feature internal type type=0 RCViewer; type=1 RCHost; type=2 Playback
	this.m_type=type;
	/// feature name
	this.m_fname=fname;
	/// feature guid
...
///log event handler
statNotifyLogMessage=function(message,severity)
{	var strseverity="";
	/*@if (@_debug)@*/
	switch(severity)
...
/// CSession class
function CSession(name,userId,password,peerId){	/// unique name session within html
	this.m_name=name;
	/// user identifier
	...
	{	if(flag)//Activate
		{	$('sraccess').disabled=false;
			$('srtype').selectedIndex=this.m_srtype;
	...
	{	if(this.m_service!=null)this.m_service.Activate(false);
		this.m_srtype=type;
		//TODO if none option in select
		this.m_type=$('srtype').options(type).value;
...
{	/// session object. it's owner.
	...
	{	if(this.m_activeX!=null){alert("activeX is not delete");return;}
		var guid="";for(prop in servicesSet)if(servicesSet[prop].m_type==0){guid=servicesSet[prop].m_guid;break;}//get guid viewer activeX
		$('activeX').outerHTML+="<OBJECT classid=\"clsid:"+guid+"\" id=\"AX"+this.m_session.m_name+"\" width=\"0%\" height=\"0%\" ></OBJECT>";//style=\"display:none\"
...
{	/// session object. it's owner.
	...
	this.Start=function()
	{	if(this.m_activeX!=null){alert("activeX is not delete");return;}
		var guid="";for(prop in servicesSet)if(servicesSet[prop].m_type==2){guid=servicesSet[prop].m_guid;break;}//get guid viewer activeX
		$('activeX').outerHTML+="<OBJECT classid=\"clsid:"+guid+"\" id=\"AX"+this.m_session.m_name+"\" width=\"100%\" height=\"100%\" ></OBJECT>";//style=\"display:none\"
		this.m_activeX = $("AX"+this.m_session.m_name);
		//this.m_activeX.attachEvent("OnDisconnect",this.OnDisconnect());
		this.AttachAXEvents();
...
{	/// session object. it's owner.
	...
	this.Start=function()
	{	if(hostObject==null)hostObjectInit();//create for demand
...
{	
	//creating host singlton ActiveX object 
	var guid="";for(prop in servicesSet)if(servicesSet[prop].m_type==1){guid=servicesSet[prop].m_guid;break;}//get guid viewer activeX
	$('activeX').outerHTML+="<OBJECT classid=\"clsid:"+guid+"\" id=\"hostObject\" width=\"0%\" height=\"0%\" ></OBJECT>";//style=\"display:none\"
	//$('spanHostObject').outerHTML+="<OBJECT classid=\"clsid:{82AA4446-50DD-443B-9AD7-919B0438E145}\" id=\"hostObject\" width=\"0%\" height=\"0%\" ></OBJECT>";//style=\"display:none\" width=\"0%\" height=\"0%\"
	...
	hostObject.attachEvent("NotifyLogMessage",statNotifyLogMessage);
...
{	/// session object. it's owner.
	...
	this.Start=function()
	{	if(this.m_activeX!=null){alert("activeX is not delete");return;}
		var guid="";for(prop in servicesSet)if(servicesSet[prop].m_type==3){guid=servicesSet[prop].m_guid;break;}//get guid viewer activeX
		$('activeX').outerHTML+="<OBJECT classid=\"clsid:"+guid+"\" id=\"AX"+this.m_session.m_name+"\" width=\"0%\" height=\"0%\" ></OBJECT>";//style=\"display:none\"
		this.m_activeX = $("AX"+this.m_session.m_name);
		this.AttachAXEvents();
		this.m_conCompleted=0;
...
{	/// session object. it's owner.
	...
	this.Start=function()
	{	if(this.m_activeX!=null){alert("activeX is not delete");return;}
		var guid="";for(prop in servicesSet)if(servicesSet[prop].m_type==4){guid=servicesSet[prop].m_guid;break;}//get guid viewer activeX
		$('activeX').outerHTML+="<OBJECT classid=\"clsid:"+guid+"\" id=\"AX"+this.m_session.m_name+"\" width=\"0%\" height=\"0%\" ></OBJECT>";//style=\"display:none\"
		this.m_activeX = $("AX"+this.m_session.m_name);
		this.AttachAXEvents();
		this.m_conCompleted=0;
...
{	/// session object. it's owner.
	this.m_session=session;
	/// activeX control object CRCInstallerAXCtrl
	//this.m_activeX=installerObject;
	...
	{	if(flag)//Activate
		{	
			$('srInstallerOptions').style.display="inline";
			//if(this.m_sid==null){$('bsrstart').disabled=false;$('srtype').disabled=false;}//if service has not connected start button is enabled
			//else {$('bsrstop').disabled=false;$('srtype').disabled=true;}//if service has connected stop button is enabled
			//if(this.m_sid!=null)
			{	$('srtype').disabled=true;
				...
				{$('activeXcon').style.display="inline";ShowConCompleted(installerObject.m_conCompleted,installerObject.m_conStatus);}
			}
			//else	
			{	$('bsrstart').disabled=false;$('srtype').disabled=false;
			}
		...
		{	$('srtype').disabled=true;
			$('srInstallerOptions').style.display="none";
			$('bsrstart').disabled=true;
			$('bsrstop').disabled=true;
			$('sraccess').disabled=true;
			$('activeXcon').style.display="none";
	...
	{	//if(this.m_sid!=null){alert("It have connected already!");return;}
		...
		installerObject.m_conStatus="start";
		$('activeXcon').style.display="inline"
		...
		//if(this.m_sid===-1)this.m_sid=null;//invalid client id
		//TODO NEW VERSION
		installerObject.InstallFeature($('srvInstallerFeatures').options($('srvInstallerFeatures').selectedIndex).innerText,$('srvInstallerFeatures').options($('srvInstallerFeatures').selectedIndex).value);
	}
	/// stop Installer service. Abort installation process
	this.Stop=function()
	{	
		installerObject.AbortInstallFeature();
	}
...
//------------------------------------------------------------------------------------------
//+static method CServiceRCInstaller class
function installerObjectInit()
{	
	// Init installer object veriable
	
	installerObject=$('installerObject');
	
	//var str=" installerObject properties:\n";
	//var array=new Array(),sortarray;
	//var i=0;
	//for(prop in installerObject)
	//	array[i++]=prop+":"+installerObject[prop]+"\t";
	//sortarray=array.sort();
	//for(i=0;i<sortarray.length;++i)str+=sortarray[i];
	//alert(str);
	if(installerObject.object==null) 
	{
		alert("installerObject did not made");
		return;
	}

	// Set mediator
	installerObject.Mediator = $('IMMediatorObject');

	//attach Logger Event 
	installerObject.attachEvent("NotifyLogMessage",statNotifyLogMessage);
	//TODO it is necessary that do dettachEvent deleting object? dettachEvent don't nowhere.
	
	/// NotifyFeatureInstalled event handler
	/// @param result if equal 0 feacher installed success, other wise install failed
	installerObject.statNotifyFeatureInstalled=function(result)
	{	
		//alert(sessions[currentSession].m_type);
		if(sessions[currentSession].m_type==5)
		{
			$('bsrstart').disabled=false;
			$('bsrstop').disabled=true;
		}
		if(result==0)window.location.reload(false);//Reloads the page from the browser cache
		else statNotifyLogMessage((new Date()).toLocaleTimeString()+"> Feature was not installed.",-1);		


	}
	installerObject.attachEvent("NotifyFeatureInstalled",installerObject.statNotifyFeatureInstalled);
			
	/// NotifyInstalling event handler.
	/// @param percent percent completed.
	/// @param message hint current process connecting.
	installerObject.statNotifyInstalling=function(percent,message)
	{	
		installerObject.m_conCompleted=percent;	
		installerObject.m_conStatus=message;
		ShowConCompleted(percent,message);
	}
	installerObject.attachEvent("NotifyInstalling",installerObject.statNotifyInstalling);	
	
	/// percent accomplish installing
	installerObject.m_conCompleted=0;
	/// current installing message
	installerObject.m_conStatus=" ";

	
	
	var s="string";
	s=installerObject.GetInstallerVersion();
	statNotifyLogMessage((new Date()).toLocaleTimeString()+"> "+"Installer version : "+s,-1);	
	...
	{	
		...
		if(i==uniqFeature.length)//new feature name unique 
		{
			...
			$('srvInstallerFeatures').options.add(oOption);
			oOption.innerText=servicesSet[prop].m_fname;
			oOption.value=servicesSet[prop].m_reqVersion;
		}
	}
	
	// search installed service
	$('srtype').options.length=0;
	for(prop in servicesSet)
	{	var guid="";
		guid=installerObject.GetFeatureID(servicesSet[prop].m_name);
		if(guid!="")
		{
			var guidver=guid.split(" ",2);
			servicesSet[prop].m_guid=guidver[0];
			servicesSet[prop].m_version=guidver[1];
			if(servicesSet[prop].m_version==servicesSet[prop].m_reqVersion||confirm(prop+" service have not actual version. Do you want to use it anyway?"))
			{
				var oOption=document.createElement("OPTION");
				$('srtype').options.add(oOption);
				oOption.innerText=prop;
				oOption.value=servicesSet[prop].m_type;
				statNotifyLogMessage((new Date()).toLocaleTimeString()+"> "+servicesSet[prop].m_name+" "+servicesSet[prop].m_guid+" "+servicesSet[prop].m_version+" ("+servicesSet[prop].m_reqVersion+")",-1);		
			}
		}
	}
}
