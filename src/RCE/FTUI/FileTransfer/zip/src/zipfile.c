...
/* we could test "if (z->dosflag && z->atx & ~0xffL)", but what for?      */
    ...
    if (z->ext || z->cext)
    {
        if (z->ext && z->cext && z->extra != z->cextra)
        {
          ...
          if (noisy) my_fprintf(stderr, "\tzip info: %s%s\n", errbuf, z->zname);
        }
...
/* or, was this warning really intended (eg. OS/2)? */
        ...
        {
            my_fprintf(stderr, "zip info: %s has %ld bytes of %sextra data\n",
...
*/
{
    ...
    while (LG(b) == LOCSIG)
    {
      ...
      *x = z;
      ...
      z->name = z->zname;
      z->cextra = z->extra;
      if (noisy) my_fprintf(mesg, "zip: reading %s\n", z->zname);
      ...
      if ((flg & 8) != 0) {
        ...
        if (s != z->siz) {
          my_fprintf(mesg, " compressed size %ld, actual size %ld for %s\n",
                  z->siz, s, z->zname);
          z->siz = s;
        }
    ...
    if (LG(b) != CENSIG && noisy) {
      my_fprintf(mesg, "zip warning: %s %s truncated.\n", zipfile,
   ...
   "Retry with option -qF to truncate, with -FF to attempt full recovery\n");
    ...
    if (zipbeg && noisy)
      my_fprintf(mesg, "%s: adjusting offsets for a preamble of %lu bytes\n",
              zipfile, zipbeg);
...
/* z->zname is used for printing and must be coded in native charset */
    ...
    if (zipbeg && noisy)
      my_fprintf(mesg, "%s: %s a preamble of %lu bytes\n",
  ...
  for (z = zfiles; z != NULL; z = z->nxt)
    if (z->mark == 1 || z->trash)
    {
      ...
      if (z->iname[z->nam - 1] != (char)0x2f) { /* don't unlink directory */
        if (verbose)
          my_fprintf(mesg, "zip diagnostic: deleting file %s\n", z->name);
        if (destroy(z->name)) {
  ...
  if (n)
  {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (i == 0 || strcmp(s[i]->name, s[i-1]->name) != 0) {
        if (verbose) {
          my_fprintf(mesg, "deleting directory %s (if empty)                \n",
                  s[i]->name);
        }
        deletedir(s[i]->name);
      }
