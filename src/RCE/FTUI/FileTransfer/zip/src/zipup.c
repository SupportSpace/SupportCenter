...
# else /* !MMAP, must be BIG_MEM */
      ...
      if (window != NULL) {
        ...
        if (remain != (ulg)q) {
          my_fprintf(mesg, " q=%lu, remain=%lu ", (ulg)q, remain);
 ...
 */
    ...
    else
    {
      while ((k = file_read(b, SBSZ)) > 0 && k != (extent) EOF)
      {
        ...
        fflush(stderr);
#else
        if (verbose) my_fprintf(stdout,"%c",'.');
#endif
      }
    }
    free((zvoid *)b);
...
#endif
  {
    ...
    if (p - o != s) {
      my_fprintf(mesg, " s=%ld, actual=%ld ", s, p-o);
  ...
  {
    if (verbose)
      my_fprintf(mesg, "\t(in=%lu) (out=%lu)", isize, s);
    if (m == DEFLATE)
      my_fprintf(mesg, " (deflated %d%%)\n", percent(isize, s));
    else
      my_fprintf(mesg, " (stored 0%%)\n");
    fflush(mesg);
  }
#ifdef WINDLL
...
#  define OBUF_SZ ZBSZ
    ...
    if (!maybe_stored) while (zstrm.avail_in != 0 && zstrm.avail_in != EOF) {
        err = deflate_zip32(&zstrm, Z_NO_FLUSH);
    ...
    do {
        err = deflate_zip32(&zstrm, Z_FINISH);
        if (maybe_stored) {
    ...
    ulg tgtsize, srcsize;  /* target and source sizes */
{
    ...
    out_total += (unsigned)deflate_zip32();
