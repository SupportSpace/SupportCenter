...
/* Issue a message for the error, clean up files and memory, and exit. */
{
  ...
  if (tempzip != NULL)
  {
    if (tempzip != zipfile) {
      ...
      my_fprintf(stderr, "attempting to restore %s to its previous state\n",
         zipfile);
...
{
  if (noisy) {
    my_fprintf(stderr, "\tzip warning: %s%s\n", a, b);
    fflush(stderr);
  }
}
...
,"  -I   don't scan through Image files"
  ...
  {
    my_printf(text[i], VERSION, REVDATE);
    putchar('\n');
  }
#ifdef DOS
  check_for_windows("Zip");
...
# endif
  ...
  {
    my_printf(versinfolines[i], "Zip", VERSION, REVDATE);
    putchar('\n');
  }
  ...
  if (strcmp(ZLIB_VERSION, zlibVersion()) == 0)
    my_printf("\tUSE_ZLIB [zlib version %s]\n", ZLIB_VERSION);
  else
    my_printf("\tUSE_ZLIB [compiled with version %s, using version %s]\n",
      ZLIB_VERSION, zlibVersion());
  i++;  /* zlib use means there IS at least one compilation option */
#endif
#if CRYPT
  my_printf("\t[encryption, version %d.%d%s of %s]\n",
            CR_MAJORVER, CR_MINORVER, CR_BETA_VER, CR_VERSION_DATE);
  for (i = 0; i < sizeof(cryptnote)/sizeof(char *); i++)
  {
    my_printf(cryptnote[i]);
    putchar('\n');
  }
...
# endif /* def VMS */
   if (result) {
     ...
     my_fprintf(mesg, "test of %s FAILED\n", zipfile);
     ziperr(ZE_TEST, "original files unmodified");
   }
   if (noisy)
     my_fprintf(mesg, "test of %s OK\n", zipfile);
}
#endif /* !WINDLL */
      ...
      if (argv[i][1])
        for (p = argv[i]+1; *p; p++)
          switch (*p)
          {
            ...
            case 'a':
              aflag = ASCII;
              my_printf("Translating to ASCII...\n");
              break;
#endif /* EBCDIC */
#ifdef CMS_MVS
            case 'B':
              bflag = 1;
              my_printf("Using binary mode...\n");
              break;
#endif /* CMS_MVS */
#ifdef TANDEM
            case 'B':
...
#       define z_tim  z->tim
           ...
           )))
      {
        ...
        if (verbose) {
          my_fprintf(mesg, "zip diagnostic: %s %s\n", z->zname,
...
# endif /* _IOBUF */
  ...
  while ((z = *w) != NULL) {
    if (z->mark == 1)
    {
      ...
      if (action != DELETE)
      {
        ...
        {
          if (action == FRESHEN)
             my_fprintf(mesg, "freshening: %s", z->zname);
          else
             my_fprintf(mesg, "updating: %s", z->zname);
          fflush(mesg);
        }
  ...
  for (f = found; f != NULL; f = fexpel(f))
  {
    ...
    if (noisy)
    {
      my_fprintf(mesg, "  adding: %s", z->zname);
      fflush(mesg);
    }
    if ((r = zipup(z, y)) != ZE_OK  && r != ZE_OPEN && r != ZE_MISS)
    {
  ...
  if (comadd)
  {
    ...
    for (z = zfiles; z != NULL; z = z->nxt)
      if (z->mark)
        ...
        {
          if (noisy)
            my_fprintf(mesg, "Enter comment for %s:\n", z->zname);
  ...
  if (verbose)
    my_fprintf(mesg, "total bytes=%lu, compressed=%lu -> %d%% savings\n",
           n, t, percent(n, t));
