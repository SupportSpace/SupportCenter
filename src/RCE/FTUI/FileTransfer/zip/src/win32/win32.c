...
int IsFileSystemOldFAT(const char *dir)
{
  ...
  DWORD vfsflags;
    ...
    if ( !GetVolumeInformation(root, NULL, 0,
                               ...
                               NULL, 0)) {
        my_fprintf(mesg, "zip diagnostic: GetVolumeInformation failed\n");
        return(FALSE);
    }
...
DWORD dwAttr;
  ...
  if ( dwAttr == 0xFFFFFFFF ) {
    my_fprintf(mesg, "zip diagnostic: GetFileAttributes failed\n");
...
#  define COMPILE_DATE ""
    ...
    my_printf(CompiledWith, COMPILER_NAME1, COMPILER_NAME2,
