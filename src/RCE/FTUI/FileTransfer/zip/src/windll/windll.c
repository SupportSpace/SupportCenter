...
extern int __far __cdecl my_printf(const char *format, ...);
extern int __far __cdecl my_fprintf(FILE *file, const char *format, ...);
...
#define DllMain_ DllEntryPoint
#endif
#ifdef WIN32
BOOL WINAPI DllMain_( HINSTANCE hInstance,
                     DWORD dwReason,
                     LPVOID plvReserved)
#else
...
int __far __cdecl my_printf(const char *format, ...)
{
va_list argptr;
HANDLE hMemory;
LPSTR pszBuffer;
...
/* my_fprintf clone for code in zip.c, etc. */
int __far __cdecl my_fprintf(FILE *file, const char *format, ...)
{
va_list argptr;
HANDLE hMemory;
LPSTR pszBuffer;
...
void __far __cdecl my_perror(const char *parm1)
{
my_printf(parm1);
}
