...
void CUDT::open(const sockaddr* addr, SOCKET win_sock)
{
   CGuard cg(m_ConnectionLock);
...
void CUDT::connect(const sockaddr* serv_addr)
{
   ...
   {
      #pragma warning( push )
	  #pragma warning( disable : 4244 )	// conversion from 'int32_t' to 'u_short', possible loss of data

      if (AF_INET == m_iIPversion)
         addr4.sin_port = static_cast<u_short>( htons(res->m_iPort) );
      else
         addr6.sin6_port = static_cast<u_short>( htons(res->m_iPort) );
