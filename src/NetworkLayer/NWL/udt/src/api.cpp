...
UDTSOCKET CUDTUnited::newSocket(const int& af, const int& type, SOCKET win_sock)
{
...
int CUDTUnited::bind(const UDTSOCKET u, const sockaddr* name, const int& namelen)
{
   ...
   s->m_pUDT->open(name, s->m_WinSocket);
...
UDTSOCKET CUDT::socket(int af, int type, int, SOCKET win_sock)
{
   try
   {
      return s_UDTUnited.newSocket(af, type, win_sock);
   }
...
namespace UDT
{
UDTSOCKET socket(int af, int type, int protocol, SOCKET win_sock)
{
   return CUDT::socket(af, type, protocol, win_sock);
}
