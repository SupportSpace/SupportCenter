...
void IGDdata(void * d, const char * data, int l)
{
	...
	if (3 == datas->level)
	{
		if( !strcmp(datas->cureltname, "friendlyName") )
			dstmember = datas->deviceInfo.m_name;
		else if( !strcmp(datas->cureltname, "manufacturer") )
			dstmember = datas->deviceInfo.m_manufacturer;
		else if( !strcmp(datas->cureltname, "manufacturerURL") )
			dstmember = datas->deviceInfo.m_manURL;
		else if( !strcmp(datas->cureltname, "modelDescription") )
			dstmember = datas->deviceInfo.m_modelDesc;
		else if( !strcmp(datas->cureltname, "modelName") )
			dstmember = datas->deviceInfo.m_model;
		else if( !strcmp(datas->cureltname, "modelURL") )
			dstmember = datas->deviceInfo.m_modelURL;
	}
	else if( !strcmp(datas->cureltname, "URLBase") )
