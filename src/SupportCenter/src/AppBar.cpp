...
// DESCRIPTION: @{HDES}
// -----------
// IMClient : CAppBar base class implementation
//---------------------------------------------------------------------------
// CHANGES LOG: @{HREV}
// -----------
// Revision: 01.00
...
CAppBar::CAppBar (UINT nIDTemplate, UINT nHtmlResID, CWnd* pParent)
     : CDHtmlDialogEx( nIDTemplate, nHtmlResID, pParent )
{
...
void CAppBar::SetState (UINT uState) {
   ...
   switch (uState) {
      ...
      case ABE_FLOAT:
		 ...
		 OutputDebugString( _T("CAppBar::SetState ABE_FLOAT") );
   ...
   //TODO:anatoly cause TrayIcons issue without SWP_NOZORDER 
   OutputDebugString( _T("CAppBar::SetState SetWindowPos2") );
   SetWindowPos(pwnd, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER );
...
UINT CAppBar::SHAppBarMessage (DWORD dwMessage, UINT uEdge /*= ABE_FLOAT*/, 
   ...
   abd.lParam           = lParam;
   UINT uRetVal         = (UINT)::SHAppBarMessage(dwMessage, &abd);
...
void CAppBar::SlideWindow (const CRect& rcEnd) {
   ...
   if (fFullDragOn && (rcStart != rcEnd)) {
      ...
      while ((dwTime = ::GetTickCount()) < dwTimeEnd) {
		 ...
		 OutputDebugString( _T("CAppBar::SlideWindow SetWindowPos1") );
	...
	OutputDebugString( _T("CAppBar::SlideWindow SetWindowPos2") );
...
void CAppBar::OnABNFullScreenApp (BOOL fOpen) {
   ...
   // Z-Order back to when the user wants it to be.
	//
	// todo anatoly - fix issue
	// the OnABNFullScreenApp is called by system too much time and we change State - this cause problem
	// when open dialog from minimized and immidiatly move/drag it
	if( m_fFullScreenAppOpen != fOpen )
	{
		m_fFullScreenAppOpen = fOpen;
		SetState();
	}
	// todo anatoly commented it - instead added condition - need attention 
	//m_fFullScreenAppOpen = fOpen;
	//SetState();
}
...
BEGIN_MESSAGE_MAP(CAppBar, CDHtmlDialogEx)//CDHtmlDialog //CDialog
...
int CAppBar::OnCreate(LPCREATESTRUCT lpCreateStruct) {
   ...
   if (CDHtmlDialogEx::OnCreate(lpCreateStruct) == -1)
      return(-1);
   ...
   // todo not required this timer as far as such feature not required
...
void CAppBar::OnTimer(UINT nIDEvent) {
   ...
   if (GetActiveWindow() != this && nIDEvent == AUTOHIDETIMERID) {
...
LRESULT CAppBar::OnNcHitTest(CPoint point) {
   ...
   LRESULT u = CDHtmlDialogEx::OnNcHitTest(point);
	//LRESULT u = CDHtmlDialog::OnNcHitTest(point);
